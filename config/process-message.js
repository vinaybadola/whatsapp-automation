...
export async function processMessages() {
  ...
  messageQueue.on('failed', async (job, error) => {
    ...
    if (error.message.toLowerCase().includes('connection closed')) {
      console.log(
        `Job ${job.id} failed due to connection closed. Delegating to retryQueue...`,
      );
    }
  });
  ...
  messageQueue.on('drained', () => {
    console.log('Queue is drained - all pending jobs processed');
  });
  ...
  messageQueue.process('sendMessage', 5, async (job) => {
    ...
    try {
      ...
      const queueProcess = true;
      const { client, shouldWait } = await connectServices.getClient(
        sessionId,
        null,
        userId,
        mode,
      ...
      await Message.findByIdAndUpdate(messageId, {
        ...
        messageTrackerId: messageTracker._id,
      });
    } catch (error) {
